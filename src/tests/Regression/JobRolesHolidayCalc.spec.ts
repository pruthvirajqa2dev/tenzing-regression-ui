...
test.describe("Regression " + `${process.env.test_env}`.toUpperCase(), () => {
    //Test case 1
    test.skip("Community - Job Roles", async ({ page }, testInfo) => {
        //Login
        ...
        const communityPage =
            await test.step(`Navigate to Commyunity`, async () => {
                return await homepage.navigateToCommunityPage(testInfo);
            });
        await communityPage.clickJobRolesLink();
        await communityPage.clickAddNewJobRoleBtn();
        //Add Job Roles Dialog
        //Verifying Header and Start Text
        await communityPage.verifyDialogHeaderText(
            expectedTexts.addJobRoleDialogHeader
        );

        await communityPage.verifyDialogStartText(
            expectedTexts.mountainBgTextForAddJobRolesDialogPart1,
            expectedTexts.mountainBgTextForAddJobRolesDialogPart2
        );
        await page
            .getByRole("dialog", { name: "Job Role" })
            .locator("i")
            .first()
            .click();
        // await communityPage.clickAddManuallyJobRoles();
        //Filling Name and Description
        const inputName = "Test Job Roles on " + new Date().getTime();
        const inputDescr =
            "This is test job role description " + new Date().getTime();
        await communityPage.fillJobRoleNameInput(inputName);
        await communityPage.clickWFCPost();
        await communityPage.selectWFCPost(expectedTexts.testWFCPost);
        await communityPage.clickWFCRole();
        await communityPage.selectWFCRole(expectedTexts.testWFCRole);

        await communityPage.clickDialogSaveAndExitBtn();
        //Uploading
        // //Verifying Header and Start Text
        // await communityPage.verifyDialogHeaderText(
        ...
        //     expectedTexts.deletePayScaleDialogContent + inputName + "?"
        // );
