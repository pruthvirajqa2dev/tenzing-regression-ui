...
test.describe("Regression " + `${process.env.test_env}`.toUpperCase(), () => {
    ...
    test("Community - Grades - Add, View, Hide, Unhide, Delete ", async ({
        page
    }, testInfo) => {
        //Login
        var loginPage = new LoginPage(page, testInfo);
        ...
        await communityPage.verifyDialogStartText(
            expectedTexts.mountainBgTextForUploadingGradesDialogPart1,
            expectedTexts.mountainBgTextForUploadingGradesDialogPart2
        );
        const filePath = path.resolve(paths.gradesPath);
        await page
            .locator(communityPage.fileUploadInputLocator)
            .setInputFiles(filePath);
        ...
        // //Done: Check the start text at preview upload dialog
        await communityPage.verifyDialogStartText(
            expectedTexts.mountainBgTextForPreviewUploadGradesDialogPart1,
            expectedTexts.mountainBgTextForPreviewUploadGradesDialogPart2
        );
        ...
        await communityPage.clickDialogSaveBtn();
        await expect(
            page.getByText(expectedTexts.gradeSuccessMsg)
        ).toBeVisible();
        ...
        await addedCard.locator(communityPage.viewBtnLocator).click();
        await communityPage.verifyDialogHeaderText(
            expectedTexts.viewGradeDialogHeaderPart + inputName
        );
        ...
        await expect(
            page.getByText(expectedTexts.gradeHiddenMsg)
        ).toBeVisible();
        ...
        await expect(
            page.getByText(expectedTexts.gradeUnhiddenMsg)
        ).toBeVisible();
        // //Toggle Show Hidden
        console.log("Toggle Show Hidden OFF");
        await communityPage.toggleShowHidden();
        ...
        await communityPage.verifyDialogHeaderContent(
            expectedTexts.deleteGradeDialogContent + inputName + "?"
        );
        await communityPage.click(communityPage.dialogDeleteButtonLocator);
        await expect(
            page.getByText(expectedTexts.gradeDeletedMsg)
        ).toBeVisible();
    });
});
